...
import numpy as np
import os
os.environ["WANDB_API_KEY"] = "782b8eaf0fe33e8155d7c7da2067295cb9d62826"
os.environ["WANDB_MODE"] = "offline"

def load_npz_data(protein_dir, atom_dir, filename):
    protein_path = os.path.join(protein_dir, filename)
    atom_path = os.path.join(atom_dir, filename)
    
    protein_data = np.load(protein_path)['protein_grid']
    atom_data = np.load(atom_path)['atom_grid']
    
    return protein_data, atom_data

def read_data(protein_dir, atom_dir, split_file):
    with open(split_file, 'r') as file:
        filenames = file.readlines()
    
    # 假设你只需要加载一个批次的数据来进行演示
    protein_batch = []
    atom_batch = []
    for filename in filenames[:10]:  # 只处理前10个文件作为示例
        filename = filename.strip()
        protein_data, atom_data = load_npz_data(protein_dir, atom_dir, filename)
        protein_batch.append(protein_data)
        atom_batch.append(atom_data)
    
    return np.array(protein_batch), np.array(atom_batch)
...
protein_dir_train = '/gpfs/share/home/2201111701/szs/szs1/voxmol/input/train_protein'
atom_dir_train ='/gpfs/share/home/2201111701/szs/szs1/voxmol/input/train_atom'
split_file_train ='/gpfs/share/home/2201111701/szs/szs1/voxmol/input/train_protein_splits.txt'

protein_dir_val = '/gpfs/share/home/2201111701/szs/szs1/voxmol/input/valid_protein'
atom_dir_val ='/gpfs/share/home/2201111701/szs/szs1/voxmol/input/valid_atom'
split_file_val ='/gpfs/share/home/2201111701/szs/szs1/voxmol/input/valid_protein_splits.txt'
...
def train(
    ...
    protein_batch, atom_batch = read_data(protein_dir_train, atom_dir_train, split_file_train)
    
    protein = torch.tensor(protein_batch, dtype=torch.float32).to('cuda')
    atom = torch.tensor(atom_batch, dtype=torch.float32).to('cuda')
    ...
    for i, batch in enumerate(loader):
        ...
        condition = voxelizer.forward(smooth_voxels + protein).to('cuda')  # 处理protein作为条件
        target = voxelizer.forward(atom).to('cuda')        # 处理atom作为真实标签
        
        # voxels = batch.to('cuda')
        # smooth_voxels = add_noise_voxels(voxels, config["smooth_sigma"])
        ...
        # update metrics
        metrics.update(loss, pred, target)
...
def val(
    ...
    protein_batch, atom_batch = read_data(protein_dir_val, atom_dir_val, split_file_val)
    protein = torch.tensor(protein_batch, dtype=torch.float32).to('cuda')
    atom = torch.tensor(atom_batch, dtype=torch.float32).to('cuda')
