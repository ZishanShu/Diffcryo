...
class CryoData_valid(Dataset):
    ...
    def __getitem__(self, idx):
        cryodata = valid[idx].strip("\n")
        file_path = os.path.join(self.root, VALID_SUB_GRIDS, cryodata)
...
def create_loader(config: dict):
    ...
    # 加载数据集
    dataset = CryoData(root="dataset_test", sub_grid_dir="train_sub_grids")
    dataset_valid = CryoData_valid(root="dataset_test", sub_grid_dir="valid_sub_grids")
    ...
    loader_train = DataLoader(dataset, batch_size=args.batch_size, shuffle=True, num_workers=1)
    loader_val = DataLoader(dataset_valid, batch_size=args.batch_size, shuffle=False, num_workers=1)
