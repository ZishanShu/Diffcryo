...
def train(
    ...
    for i, (protein, amino) in enumerate(loader):
        t = torch.randint(0, model.module.timesteps, (amino.size(0),), device='cuda').float()
        protein = protein.unsqueeze(1)
        amino = amino.unsqueeze(1)
        
        protein = protein.to('cuda')
        amino = amino.to('cuda')
        
        # forward/backward
        output = model.module.diffusion_step(amino, t, protein) # (batch_size, 4, 32, 32, 32)
        ...
        loss = criterion(output, amino.squeeze(1).long())  # 直接使用amino作为真实值
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
        accuracy = (preds == amino).float().mean()  # 计算准确率

        total_loss += loss.item()
        num_batches += 1
        
        # update metrics
        metrics.update(output, amino.squeeze(1))
...
def val(
    ...
    with torch.no_grad():
        for i, (protein, amino) in enumerate(loader):
            ...
            amino = amino.unsqueeze(1) # [batch_size, 1, 32, 32, 32]
            
            protein = protein.to('cuda')
            amino = amino.to('cuda')
            
            # forward
            output = model.module.diffusion_step(amino, t, protein)
            preds = model.module.post_process_output(output) # (batch_size, 32, 32, 32)
