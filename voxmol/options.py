...
def parse_args():
    ...
    parser.add_argument(
        "--exp_name", default="cryodiff_experiment", type=str, help="experiment name"
    )
    parser.add_argument(
        "--wandb", default=1, type=int, help="use wandb if > 0"
    )
    ...
    parser.add_argument(
        "--dset_name", default="emdb", type=str, help="dataset name (qm9 | drugs)"
    )
    parser.add_argument(
    ...
    assert config["dset_name"] in ["emdb"], "dataset not supported"
    ...
    with open("/gpfs/share/home/2201111701/szs/szs1/voxmol/voxmol/models/configs/unet3d_config.yml", "r") as f:
    ...
    # set default 'elements', 'grid_dim' and 'num_channels' according to the dataset
    if config["dset_name"] == "emdb":
        config["elements"] = ["CA", "C", "N"]
    config["grid_dim"] = 32
    config["num_channels"] = len(config["elements"])   
